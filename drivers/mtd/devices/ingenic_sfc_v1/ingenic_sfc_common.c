...
static void sfc_glb_info_config(struct sfc *sfc, struct sfc_transfer *transfer)
{
	...
	if((transfer->ops_mode == DMA_OPS)){
		if(transfer->direction == GLB_TRAN_DIR_READ)
			dma_cache_inv((unsigned long)transfer->data, transfer->len);
		else
			dma_cache_wback((unsigned long)transfer->data, transfer->len);
...
struct sfc *sfc_res_init(struct platform_device *pdev)
{
	...
	if (IS_ERR(sfc->clk))
		sfc->clk = devm_clk_get(&pdev->dev, "div_sfc0");
	if (IS_ERR(sfc->clk))
		sfc->clk = devm_clk_get(&pdev->dev, "div_sfc");
	if (IS_ERR(sfc->clk))
		sfc->clk = clk_get(NULL, "div_sfc0");
	if (IS_ERR(sfc->clk))
		sfc->clk = clk_get(NULL, "div_sfc");
	if (IS_ERR(sfc->clk)) {
		dev_err(&pdev->dev, "Cannot get SFC cgu clock\n");
		return ERR_PTR(-ENOENT);
	}

	sfc->clk_gate = devm_clk_get(&pdev->dev, "gate_sfc");
	if (IS_ERR(sfc->clk_gate))
		sfc->clk_gate = devm_clk_get(&pdev->dev, "gate_sfc0");
	if (IS_ERR(sfc->clk_gate))
		sfc->clk_gate = clk_get(NULL, "gate_sfc0");
	if (IS_ERR(sfc->clk_gate))
		sfc->clk_gate = clk_get(NULL, "gate_sfc");
	if (IS_ERR(sfc->clk_gate)) {
		dev_err(&pdev->dev, "Cannot get SFC gate clock\n");
		return ERR_PTR(-ENOENT);
	}
	...
	/*
	 * Don't call clk_set_rate — U-Boot already configured the SFC clock
	 * divider correctly (MPLL/12 = 100MHz). The clock framework's
	 * set_rate toggles CE/STOP bits which can glitch the SFC controller.
	 * Just keep what U-Boot set.
	 */
	dev_info(&pdev->dev, "SFC clk rate: %lu Hz (keeping U-Boot setting)\n",
