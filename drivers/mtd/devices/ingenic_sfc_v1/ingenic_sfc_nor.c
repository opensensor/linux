...
static int __init ingenic_sfc_probe(struct platform_device *pdev)
{
	...
	/* Try burner params first */
	pdata_params = pdev->dev.platform_data;
	ret = ingenic_spi_norflash_get_params(flash, pdata_params);
	if (ret) {
		/* No burner params — use JEDEC ID and defaults (thingino mode) */
		static struct spi_nor_info default_nor_info;
		static struct norflash_partitions default_partitions;
		static struct nor_private_data default_pri_data;
		unsigned int chip_id;

		chip_id = sfc_nor_read_id(flash, 0x9f, 0, 0, 3, 0);
		dev_info(flash->dev, "JEDEC ID: 0x%06x\n", chip_id);
		if (chip_id == 0 || chip_id == 0xffffff) {
			ret = -ENODEV;
			dev_err(flash->dev, "No valid NOR flash detected!\n");
			goto err_match_device;
		}

		memset(&default_nor_info, 0, sizeof(default_nor_info));
		snprintf(default_nor_info.name, sizeof(default_nor_info.name), "spi-nor-%06x", chip_id);
		default_nor_info.id = chip_id;
		default_nor_info.page_size = 256;
		default_nor_info.erase_size = 4096;
		default_nor_info.chip_erase_cmd = 0xc7;

		/* Detect size from JEDEC ID byte 3 */
		switch (chip_id & 0xff) {
		case 0x20: default_nor_info.chip_size = 0x100000; break;   /* 1MB */
		case 0x19: default_nor_info.chip_size = 0x2000000; break;  /* 32MB */
		case 0x18: default_nor_info.chip_size = 0x1000000; break;  /* 16MB */
		case 0x17: default_nor_info.chip_size = 0x800000; break;   /* 8MB */
		case 0x16: default_nor_info.chip_size = 0x400000; break;   /* 4MB */
		case 0x15: default_nor_info.chip_size = 0x200000; break;   /* 2MB */
		default:   default_nor_info.chip_size = 0x1000000; break;  /* default 16MB */
		}

		/* Standard SPI read/write commands */
		default_nor_info.read_standard.cmd = 0x03;
		default_nor_info.read_standard.dummy_byte = 0;
		default_nor_info.read_standard.addr_nbyte = 3;
		default_nor_info.read_standard.transfer_mode = TM_STD_SPI;

		default_nor_info.write_standard.cmd = 0x02;
		default_nor_info.write_standard.dummy_byte = 0;
		default_nor_info.write_standard.addr_nbyte = 3;
		default_nor_info.write_standard.transfer_mode = TM_STD_SPI;

		default_nor_info.sector_erase.cmd = 0x20;
		default_nor_info.sector_erase.dummy_byte = 0;
		default_nor_info.sector_erase.addr_nbyte = 3;
		default_nor_info.sector_erase.transfer_mode = TM_STD_SPI;

		default_nor_info.wr_en.cmd = 0x06;
		default_nor_info.wr_en.dummy_byte = 0;
		default_nor_info.wr_en.addr_nbyte = 0;
		default_nor_info.wr_en.transfer_mode = TM_STD_SPI;

		default_nor_info.busy.cmd = 0x05;
		default_nor_info.busy.bit_shift = 0;
		default_nor_info.busy.mask = 0x1;
		default_nor_info.busy.val = 0;
		default_nor_info.busy.len = 1;
		default_nor_info.busy.dummy = 0;

		default_nor_info.tCHSH = DEF_TCHSH;
		default_nor_info.tSLCH = DEF_TSLCH;
		default_nor_info.tSHSL_RD = DEF_TSHSL_R;
		default_nor_info.tSHSL_WR = DEF_TSHSL_W;

		default_pri_data.fs_erase_size = 32768;
		default_pri_data.uk_quad = 0;

		default_partitions.num_partition_info = 0;

		nor_info->nor_flash_info = &default_nor_info;
		nor_info->norflash_partitions = &default_partitions;
		nor_info->nor_pri_data = &default_pri_data;

		dev_info(flash->dev, "Using JEDEC defaults: %s, %uKB\n",
			 default_nor_info.name, default_nor_info.chip_size / 1024);
	}
	...
	ret = mtd_device_parse_register(&flash->mtd, ingenic_probe_types, NULL, NULL, 0);
	if (ret) {
		ret = -ENODEV;
