...
#include "pinctrl-ingenic-xburst.h"

static struct ingenic_pinctrl *gpctl = NULL;
static const struct ingenic_priv common_priv_data[];

/*
 * gpio_to_chip was removed in kernel 6.x. Replicate it using our global
 * pinctrl state — convert a global GPIO number to its gpio_chip.
 */
static inline struct gpio_chip *gpio_to_chip(unsigned int gpio)
{
	...
	if (!gpctl)
		return NULL;
	for (i = 0; i < gpctl->num_chips; i++) {
		struct gpio_chip *gc = &gpctl->gpio_chips[i].gc;

		if (gpio >= gc->base && gpio < gc->base + gc->ngpio)
			return gc;
	}
	return NULL;
}
...
struct ingenic_gpio_chip *ingenic_get_gpio_chip(struct gpio_chip *chip)
{
	return container_of(chip, struct ingenic_gpio_chip, gc);
}

unsigned int ingenic_get_used_pins_bitmap(struct gpio_chip *chip)
{
	struct ingenic_gpio_chip *jzgc = ingenic_get_gpio_chip(chip);
	return jzgc->used_pins_bitmap;
}

void ingenic_set_used_pins_bitmap(struct gpio_chip *chip, unsigned int bitmap)
{
	struct ingenic_gpio_chip *jzgc = ingenic_get_gpio_chip(chip);
	jzgc->used_pins_bitmap = bitmap;
}
#endif
		...
		enum gpio_function func, u32 pins)
{
	...
	/*shadow mode step 1:*/
	ingenic_gpio_writel(chip, PxSHDS, 1);
	...
	/*shadow mode step 2:*/
	ingenic_gpio_writel(chip, PxUPD, 1);
	/*shadow mode step 3:*/
	ingenic_gpio_writel(chip, PxSHDC, 1);
 ...
 *	chip:		gpio chip
 *	pins:		pins bitmap
 *	strength:	gpio drive_strength bit set
 **************************************************************/
#ifdef CONFIG_SOC_PRJ007
static void ingenic_gpio_set_drive_strength(struct ingenic_gpio_chip *chip,
		u32 pin, u16 strength)
{
	if (strength != 0 && strength != 4 && strength != 8 && strength != 12) {
		strength = 4;
		printk("driver strength configuration error,only support 4mA 8mA 12mA,now we use default 4mA\n");
	}
	if (pin > 15) {
		pin = pin - 16;
		if (4 == strength) {
...
#else
static void ingenic_gpio_set_drive_strength(struct ingenic_gpio_chip *chip,
		u32 pin, u16 strength_mode)
{
	if (strength_mode != 0 && strength_mode != 1 && strength_mode != 2 && strength_mode != 3 && strength_mode != 4) {
		strength_mode = 1;
		printk("driver strength_mode configuration error,only support mode 1 2 3 4,now use default mode 1\n");
	}
	if (1 == strength_mode) {
		ingenic_gpio_writel(chip, PxPDS1C, (1 << pin));
		ingenic_gpio_writel(chip, PxPDS0C, (1 << pin));
	} else if (2 == strength_mode) {
		ingenic_gpio_writel(chip, PxPDS1C, (1 << pin));
		ingenic_gpio_writel(chip, PxPDS0S, (1 << pin));
	} else if (3 == strength_mode) {
		ingenic_gpio_writel(chip, PxPDS1S, (1 << pin));
		ingenic_gpio_writel(chip, PxPDS0C, (1 << pin));
	} else if (4 == strength_mode) {
		ingenic_gpio_writel(chip, PxPDS1S, (1 << pin));
		ingenic_gpio_writel(chip, PxPDS0S, (1 << pin));
	}else {
		ingenic_gpio_writel(chip, PxPDS1C, (1 << pin));
...
static unsigned short ingenic_gpio_get_drive_strength(struct ingenic_gpio_chip *chip,
		u32 pin)
{
	unsigned short strength_mode = 0;
	unsigned short strength_high = 0;
	unsigned short strength_low = 0;
	strength_high = (ingenic_gpio_readl(chip, PxPDS1) & BIT(pin)) ? (1 << 0) : 0;
	strength_low = (ingenic_gpio_readl(chip, PxPDS0) & BIT(pin)) ? (1 << 0) : 0;
	if ((0 == strength_high) && (0 == strength_low)) {
		strength_mode = 1;
	} else if((0 == strength_high) && (1 == strength_low)) {
		strength_mode = 2;
	} else if((1 == strength_high) && (0 == strength_low)) {
		strength_mode = 3;
	} else if((1 == strength_high) && (1 == strength_low)) {
		strength_mode = 4;
	}
	return strength_mode;
}
#endif
...
static int ingenic_gpio_irq_set_type(struct irq_data *irqd, unsigned int flow_type)
{
	...
	ingenic_gpio_set_func(jzgc, func, BIT(irqd->hwirq));
	return 0;
}
...
static int ingenic_gpio_set(struct gpio_chip *chip,
		...
		struct device_node *np, int base, int idx)
{
	...
	return gpiochip_add_data(gc, jzgc);
}
...
static int ingenic_gc_match(struct gpio_chip *chip, const void *data)
{
	return chip->fwnode == of_fwnode_handle((struct device_node *)data);
}

static struct gpio_chip *ingenic_find_gpiochip(struct device_node *np)
{
	struct gpio_device *gdev;

	gdev = gpio_device_find(np, ingenic_gc_match);
	if (!gdev)
		return NULL;
		...
		struct pinctrl_map *map, int num_maps)
{
	...
	while (!of_parse_phandle_with_args(np, "ingenic,pincfg",
				...
				&out_args)) {
		gc = ingenic_find_gpiochip(out_args.np);
		if (!gc) return -EINVAL;
		...
		unsigned group_selector)
{
	...
	ingenic_gpio_set_func(jzgc, grp->pinmux_func, grp->pinmux_bitmap);
		...
		bool input)
{
	...
	ingenic_gpio_set_func(jzgc, func, BIT(pin));
	return 0;
}
		...
		struct ingenic_pinctrl_group *grp)
{
	...
	if (!gpio_np)
		return 0;
	grp->gc = ingenic_find_gpiochip(gpio_np);
...
static int ingenic_gpio_suspend_set(struct ingenic_gpio_chip *jzgc)
{
	...
	if (pm_pullup) {
		ingenic_gpio_set_func(jzgc, GPIO_INPUT, pm_pullup);
		ingenic_gpio_set_pull(jzgc, pm_pullup, INGENIC_GPIO_PULLUP);
	}
	if (pm_pulldown) {
		ingenic_gpio_set_func(jzgc, GPIO_INPUT, pm_pulldown);
		ingenic_gpio_set_pull(jzgc, pm_pulldown, INGENIC_GPIO_PULLDOWN);
	}
	if(pm_hiz) {
		ingenic_gpio_set_func(jzgc, GPIO_INPUT, pm_hiz);
		ingenic_gpio_set_pull(jzgc, pm_hiz, INGENIC_GPIO_HIZ);
	}

	if (pm_high)
		ingenic_gpio_set_func(jzgc, GPIO_OUTPUT1, pm_high);
	if (pm_low)
		ingenic_gpio_set_func(jzgc, GPIO_OUTPUT0, pm_low);

	return 0;
}

static int ingenic_pinctrl_suspend(void *data)
{
	struct ingenic_pinctrl *pctl = gpctl;
	int idx;
...
static void ingenic_pinctrl_resume(void *data)
{
	struct ingenic_pinctrl *pctl = gpctl;
	int idx;
...
static struct syscore ingenic_pinctrl_syscore = {
	.ops = &ingenic_pinctrl_syscore_ops,
};
...
static const struct of_device_id ingenic_pinctrl_dt_match[] = {
	...
	{ .compatible = "ingenic,PRJ007-pinctrl", .data = (void *)common_priv_data},
	{ .compatible = "ingenic,PRJ008-pinctrl", .data = (void *)common_priv_data},
	{},
};
...
{
	register_syscore(&ingenic_pinctrl_syscore);
...
{
	unregister_syscore(&ingenic_pinctrl_syscore);
