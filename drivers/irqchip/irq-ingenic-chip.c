...
static int __init ingenic_intc_of_init(struct device_node *node)
{
	...
	/* Single-core fallback: if no cpu-intc-map in DT, use iobase directly */
	if (index == 0) {
		cpu_intc_map[0].cpu_num = 0;
		cpu_intc_map[0].dev_base = (unsigned int)iobase;
	}

	core_irq_setup(irq_chips);

	domain = irq_domain_create_legacy(of_fwnode_handle(node), INTC_NR_IRQS, 8, 0, &ingenic_irq_domain_ops, chip);
...
IRQCHIP_DECLARE(ingenic_intc2, "ingenic,ingenic-intc", ingenic_intc_init);
