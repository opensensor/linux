...
#include <soc/cpm.h>
#include <soc/base.h>
#if defined (CONFIG_CLK_PRJ007)
#include <dt-bindings/clock/ingenic-PRJ007.h>
#elif defined (CONFIG_CLK_PRJ008)
...
#ifdef CPM_TNPUCDR
			if(rd->offset == CPM_TNPUCDR){
				bit =  CLK_GATE_TNPU - CLK_ID_GATE;
				cpm_clear_bit(bit % 32, clkgr[bit / 32]);
			}
#endif

#ifdef SDHCI_CLOCK_CONTROL
			if(rd->offset == CPM_MSC0CDR){
				//The clock control needs to be enabled before restoring the MSC0 clock.
				outl(0x7, MSC0_IOBASE + SDHCI_CLOCK_CONTROL);
			}

			if(rd->offset == CPM_MSC1CDR){
				//The clock control needs to be enabled before restoring the MSC1 clock.
				outl(0x7, MSC1_IOBASE + SDHCI_CLOCK_CONTROL);
			}
#endif
...
#ifdef CONFIG_PM
static int ingenic_clk_suspend(void *data)
{
...
static void ingenic_clk_resume(void *data)
{
	struct ingenic_clock_reg_cache *reg_cache;
...
static const struct syscore_ops ingenic_clk_syscore_ops = {
	.suspend = ingenic_clk_suspend,
	.resume = ingenic_clk_resume,
};

static struct syscore ingenic_clk_syscore = {
	.ops = &ingenic_clk_syscore_ops,
};
		...
		unsigned long nr_reg_sleep)
{
	...
	if (list_empty(&clock_reg_cache_list))
		register_syscore(&ingenic_clk_syscore);
