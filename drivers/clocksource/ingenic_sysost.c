...
#ifdef CONFIG_PM
struct tmr_ost_reg{
	unsigned int val_tccr;
	unsigned int val_ter;
	unsigned int val_tesr;
	unsigned int val_tecr;
	unsigned int val_tcr;
	unsigned int val_tfr;
	unsigned int val_tmr;
	unsigned int val_t1dfr;
	unsigned int val_t1cnt;
	unsigned int val_t2cntl;
	unsigned int val_t2cnth;
	unsigned int val_tcnt2hbuf;
};
u64 ost_cnt_record = 0;
#endif/*CONFIG_PM*/
...
#ifdef CONFIG_PM
	return cycle.cycle64+ost_cnt_record;
#else
...
static void tmr_src_suspend(struct clocksource *cs)
{
	...
	do{
		i --;
		cycle.cycle32[0] = ost_readl(tmr->iobase + OST_T2CNTL);
		cycle.cycle32[1] = ost_readl(tmr->iobase + OST_T2CNTH);
	}while(cycle.cycle32[1] != ost_readl(tmr->iobase + OST_TCNT2HBUF) && i);
	if(0 == i){
		printk("suspend: get cycles error!!!");
	}
	ost_cnt_record += cycle.cycle64;

	tmr->pm.val_tccr = ost_readl(tmr->iobase + OST_TCCR);
	tmr->pm.val_ter = ost_readl(tmr->iobase + OST_TER);
	tmr->pm.val_tesr = ost_readl(tmr->iobase + OST_TESR);
	tmr->pm.val_tecr = ost_readl(tmr->iobase + OST_TECR);
	tmr->pm.val_tcr = ost_readl(tmr->iobase + OST_TCR);
	tmr->pm.val_tfr = ost_readl(tmr->iobase + OST_TFR);
	tmr->pm.val_tmr = ost_readl(tmr->iobase + OST_TMR);
	tmr->pm.val_t1dfr = ost_readl(tmr->iobase + OST_T1DFR);
	tmr_src_disable(cs);
	if(tmr->clk_gate)
...
static void tmr_src_resume(struct clocksource *cs)
{
	...
	ost_writel(tmr->iobase + OST_TER, tmr->pm.val_ter);
	ost_writel(tmr->iobase + OST_TESR, tmr->pm.val_tesr|(1 << CLKSOURCE_CH));
	ost_writel(tmr->iobase + OST_TECR, tmr->pm.val_tecr);
	ost_writel(tmr->iobase + OST_TCR, tmr->pm.val_tcr | (1<<CLKEVENT_CH));
	ost_writel(tmr->iobase + OST_TFR, tmr->pm.val_tfr);
	ost_writel(tmr->iobase + OST_TMR, tmr->pm.val_tmr | (1 << CLKEVENT_CH));
	ost_writel(tmr->iobase + OST_T1DFR, tmr->pm.val_t1dfr);
	ost_writel(tmr->iobase + OST_TCCR, tmr->pm.val_tccr);
	tmr_src_enable(cs);
#endif/*CONFIG_PM*/
}
...
TIMER_OF_DECLARE(PRJ007_ost_init, "ingenic,PRJ007-ost", ingenic_ost_init);
TIMER_OF_DECLARE(PRJ008_ost_init, "ingenic,PRJ008-ost", ingenic_ost_init);
