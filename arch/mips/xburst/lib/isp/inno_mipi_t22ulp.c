...
struct tx_isp_subdev *isp_i2c_new_subdev_board_1(struct i2c_adapter *adapter, struct i2c_board_info *info,
...
static int inno_mipi_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = inno_mipi_write(client, 0x1d, 0x33);
	if(ret) {
		printk("inno_mipi_setting() error line : %d i2c addr is 0x%x!\n",__LINE__,i2c_addr);
		goto error;
	}

	ret = inno_mipi_write(client, 0xff, 0x01);
	if(ret) {
		printk("inno_mipi_setting() error line : %d i2c addr is 0x%x!\n",__LINE__,i2c_addr);
		goto error;
	}
...
int init_inno_mipi(void)
{
   ...
   /* usleep_range(100000, 200000);  */
        ...
        isp_i2c_new_subdev_board_1(adapter, &inno_mipi_board_info, NULL);
