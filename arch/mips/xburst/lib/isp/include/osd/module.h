...
struct Module {
	...
	int (*OnDataUpdateLeft)(void *modulex, void *data);
...
#if 0
static inline int nr_module_empty_msg(Module *module)
{
	int sem_msg_cnt, sem_cnt, cnt;

	sem_getvalue(&module->sem_msg, &sem_msg_cnt);
	sem_getvalue(&module->sem, &sem_cnt);
	sem_msg_cnt = MAX_MODULE_MSG_DEPTH - sem_msg_cnt;
	cnt = sem_msg_cnt > sem_cnt ? sem_msg_cnt : sem_cnt;
...
static inline void SetUpdateLeftCallback(Module *module, int (*update_left_cb)(void *, void *))
{
    if (module->num_of_observer) {
        module->OnDataUpdateLeft = update_left_cb;
    }
}
