...
struct ddr_phy_resume_reg {
	...
	unsigned int odt_pd;
	unsigned int odt_pu;
	unsigned int drvcmd_pd;
	unsigned int drvcmd_pu;
	unsigned int drvclk_pd;
	unsigned int drvclk_pu;
	unsigned int drval_pd;
	unsigned int drval_pu;
	unsigned int drvah_pd;
	unsigned int drvah_pu;
};
...
int  soc_pm_fastboot_config(void);
void soc_pm_wakeup_fastboot(void);
void load_func_to_oram(void);
void sys_save(void);
void sys_restore(void);
void get_compilation_time(void);
void setting_debug_mode(int mode);
void ingenic_fastboot_deinit(void);
int ingenic_fastboot_init(void);
/*
 * Wakeup pin effective minimum time in number of 32 RTCLK cycles, used as
 * glitch filter logic. Maximum of 2 seconds if the RTCLK is 32768Hz
 * If this value is configured to 0, and the WKUP pin keeps low longer than 15 RTCLK periods,
 * it will wakes up RTC from Hibernation. The glitch filter time equals to HWFCR*32 RTCLK period.
 *
 * Default can be left unset.
 *
 * */
int soc_pm_set_hwfcr_cycles(int cs);
/*
 * HIBERNATE Reset asserting time.
 * Tassert_time = (HRCR+1) * 512 * Trtclk.if TRTCLK HRCR<=14
 * Tassert_time = (2^14)* Trtclk .if HRCR=15
 * TRTCLK is RTCLK period
 *
 * Default can be left unset
 * */
int soc_pm_set_hwcr_cycles(int cs);
#endif
