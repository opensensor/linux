...
int PRJ008_pm_enter(suspend_state_t state)
{
	unsigned int sleep_addr = 0;
	unsigned int ret =0;

	/* CRITICAL: Disable interrupts before entering sleep code
	 * Sleep code will flush cache and access DDR PHY registers
	 * Any interrupt during this process could access kernel code in DDR
	 * which will be in self-refresh mode
	 */
	ret = soc_pm_fastboot();
	if(ret < 0){
		return ret;
...
static int PRJ008_pm_begin(suspend_state_t state)
{
	printk("PRJ008 suspend begin\n");
	pm_suspend_start_time = jiffies;
	return 0;
}

static void PRJ008_pm_end(void)
{
	printk("PRJ008 pm end!\n");
}
#if defined CONFIG_KP_AXP
static int PRJ008_suspend_prepare(void)
{
	return regulator_suspend_prepare(PM_SUSPEND_MEM);
}
static void PRJ008_suspend_finish(void)
{
...
static const struct platform_suspend_ops PRJ008_pm_ops = {
	.valid		= ingenic_pm_valid,
	.begin		= PRJ008_pm_begin,
	.enter		= PRJ008_pm_enter,
	.end		= PRJ008_pm_end,
#if defined CONFIG_KP_AXP
	.prepare = PRJ008_suspend_prepare,
	.finish = PRJ008_suspend_finish,
#endif
};
