...
{
	printk("Restarting after 500 ms\n");
	local_irq_disable();
#ifdef CONFIG_ZERATUL
	if ((reset_type == 3) || ((command != NULL) && !strcmp(command, "recovery"))) {
...
static int type_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m,"%s", reset_type ? reset_command[reset_type - 1] : "null");
	return len;
}

static int type_write_proc(struct file *file, const char __user *buffer,
...
static struct proc_ops type_proc_ops ={
	.proc_open = reset_open_proc,
	.proc_read = seq_read,
	.proc_write = type_write_proc,
};
...
static int __init init_reset(void)
{
	...
	res = proc_create_data("reset", 0600, p, &reset_proc_ops, wdt);
#ifdef CONFIG_ZERATUL
	res =  proc_create_data("type", 0444,p, &type_proc_ops, wdt);
#endif/*CONFIG_ZERATUL*/
